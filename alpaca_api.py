...
class AlpacaAPI:
    ...
    def place_limit_order(self, symbol: str, notional: float, side: str,
                           limit_price: float, time_in_force=None):
        """
        Place a limit order at limit_price.
        Falls back to a market order if limit_price is None.
        """
        ...
        def _call():
            ...
            order = self.trading_client.submit_order(order_data)
            logger.info(f"LIMIT ORDER: {side.upper()} {symbol} @ ${limit_price:.2f} | ${notional:.2f}")
            return self._order_to_dict(order)
        return _with_retry(_call)
